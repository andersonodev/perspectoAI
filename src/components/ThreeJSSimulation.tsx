...
const ThreeJSSimulation = ({ topic = "Conceito 3D" }: ThreeJSSimulationProps) => {
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          <Cube className="h-5 w-5" />
          <span>Simulação Visual: {topic}</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg p-8 text-center">
          <Cube className="h-16 w-16 mx-auto mb-4 text-blue-600" />
          <h3 className="text-lg font-semibold mb-2">Simulação 3D</h3>
          <p className="text-gray-600 dark:text-gray-400">
            Visualização interativa para {topic}
          </p>
